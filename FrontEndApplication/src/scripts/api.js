//to avoid acomplishing we'll use routes 
const routes = {
  //http://localhost:8080/champions/1/ask
...
const apiService = {
  ...
  async postAskChampion(id,mesage){
    const route = routes.ask.replace("{id}", id);
    //setting the requisition type
    const options = {
      method:"POST",
      headers:{
...
async function renderChampions(){
  ...
  //getting each champions informations
  const elements = championsData.map((characters)=>`<div class="timeline-carousel__item" onclick="onChangeChampionSelected(${characters.id},'${characters.imageUrl}')">
  <div class="timeline-carousel__image">
    <div class="media-wrapper media-wrapper--overlay"
      style="background: url('${characters.imageUrl}') center center; background-size:cover;">
    </div>
  </div>
  <div class="timeline-carousel__item-inner">
    <span class="name">${characters.name}</span>
    <span class="role">${characters.role}</span>
    <p>${characters.lore}</p>
  </div>
</div>`);
  //injectinc the champions infos on html
...
async function getRandomQuote(){
  //fazer com que ele faça uma pergunta "desafie o usuário a te perguntar algo"
  const quotes = [
    "Manda Ver meu nobre",
...
async function fatchAskChampion(){
  document.body.style.cursor = "wait";
  const id = state.views.avatar.dataset.id;
  const mesage = state.views.question.value;
  const response= await apiService.postAskChampion(id,mesage);
